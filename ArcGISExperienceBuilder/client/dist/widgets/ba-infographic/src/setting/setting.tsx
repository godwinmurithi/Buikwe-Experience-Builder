/** @jsx jsx */
import { React, jsx, css, getAppStore, Immutable, ImmutableArray, proxyUtils, SupportedUtilityType, UseUtility } from 'jimu-core'
...
//import { showCheckbox } from 'jimu-ui/basic/lib/list-tree/common/tree-utils'

// import { ThemeBackgroundSetting } from 'jimu-ui/advanced/style-setting-components'
//import BAMapActions from '../runtime/ba-map-actions'
...
export default class Setting extends React.PureComponent<AllWidgetSettingProps<any>, any> {
  ...
  // igSettingsDebouncer: Debounce
  // handleIgSettingChange: any
  //handleIgSettingChange: any
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      geographiesChecked: props.config.geographies,
    ...
    // this.igSettingsDebouncer = new Debounce(this)
    // this.handleIgSettingChange = (name, value) => {
    //   console.log(name, value)
    // }
    // this.handleIgSettingChange = (name: string, value: any) => {
    //   this.igSettingsDebouncer.debounce(this.handleIgSettingChange, 6000)
    // }
  }
  ...
  setDefaults () {
    ...
    if (ACLUtils.notDef(this.props.config.viewMode)) {
      changeArr.push({ name: 'viewMode', value: ViewMode.Full })
    }
    if (ACLUtils.notDef(this.props.config.sourceCountry)) {
      changeArr.push({ name: 'sourceCountry', value: 'US' })
    }
    if (ACLUtils.notDef(this.props.config.workflowIntroText)) {
      changeArr.push({ name: 'workflowIntroText', value: this.localeString('introText') })
    }
    if (ACLUtils.notDef(this.props.config.workflowIntroTextReports)) {
      changeArr.push({ name: 'workflowIntroTextReports', value: this.localeString('infographicDesc') })
    }
    if (ACLUtils.notDef(this.props.config.workflowIntroTextBuffers)) {
    ...
    if (ACLUtils.notDef(this.props.config.backgroundColor)) {
      changeArr.push({ name: 'backgroundColor', value: '#525659' })
    }
    ...
    if (ACLUtils.notDef(this.props.config.headerColor)) {
      changeArr.push({ name: 'headerColor', value: '#0079C1' })
    }
    ...
    if (ACLUtils.notDef(this.props.config.print)) {
      changeArr.push({ name: 'print', value: true })
    }
    ...
    if (ACLUtils.notDef(this.props.config.pointsOfInterest)) {
      changeArr.push({ name: 'pointsOfInterest', value: true })
    }
    ...
    if (ACLUtils.notDef(this.props.config.geographies)) {
      changeArr.push({ name: 'geographies', value: true })
    }
    if (ACLUtils.notDef(this.props.config.noGeographySearch)) {
      changeArr.push({ name: 'noGeographySearch', value: false })
    }
  ...
  reportCheckedHandler (ev: any) {
    const report = ev.detail.detail.report[0]
    const reportList = this.props.config.reportList
    const isChecked = report.isChecked

    const newPublicList = reportList.public.map(item => {
      if (item.id === report.id) {
        return { ...item, isChecked }
      }
      return item
    })
    const newUserList = reportList.user.map(item => {
      if (item.id === report.id) {
        return { ...item, isChecked }
      }
      return item
    })
    const newSharedList = reportList.shared.map(item => {
      if (item.id === report.id) {
        return { ...item, isChecked }
      }
      return item
  ...
  componentDidMount () {
    this.preloadData()
    const self = this
    // document.getElementById('tooltip_save').addEventListener('click', function () {
    //   self.copyStateToGlobalConfig()
    // })

    const langCode = getAppStore().getState().appContext.locale || 'en'

    fetch('https://geoenrich.arcgis.com/arcgis/rest/services/World/GeoEnrichmentServer/Geoenrichment/countries?f=pjson&langCode=' + langCode)
  ...
  updateSelectedCountry = (country) => {
    ...
    requestAnimationFrame(() => {
      ...
      const cfg = self.props.config.set('selectedGeographyLevels', availableLevels)
        .set('availableGeographyLevels', availableLevels)
        .set('langCode', getAppStore().getState().appContext.locale || 'en')
        .set('sourceCountry', country)

      // use only 1 setting change call for all 4 config updates here
      self.props.onSettingChange({
        id: self.props.id,
        config: cfg
      })

      // sync the setting local UI state (triggers update)
  ...
  componentDidUpdate (prevProps, prevState) {
    const { widgetMode, id, sourceCountry, selectedGeographyLevels, geographies } = this.props.config
    const { selectedCountry, activeGeographyLevels, geographiesChecked } = this.state
    ...
    if (geographiesChecked !== geographies) {
      this.updateState('geographiesChecked', geographies)
    }

    const elem = document.getElementById(id + '_' + 'reports1')
    if (this.props.config !== prevProps.config) {
      this.preloadData()
    }
    if (elem) {
      let flag: boolean = false
      if (widgetMode === Mode.Workflow) { flag = true }

      // @ts-expect-error
      elem.setMultipleChoice(flag)
    }
  }
  ...
  toggleSidePopper = (name: string) => {
    ...
    if (widgetMode === Mode.Preset) {
      ...
      showCheckboxes = workflowEnableInfographicChoice
    }

    /** IMPORTANT: Makes sure the UI is updated */
    requestAnimationFrame(() => {
      const { user, token } = this.props
      const { sourceCountry, reportList } = this.props.config
      const elem = document.getElementById(self.props.id + '_' + 'reports1')

      const langCode = getAppStore().getState().appContext.locale || 'en'

      if (elem) {
        // @ts-expect-error
        elem.initialize(user.username, token, sourceCountry, langCode, showCheckboxes, JSON.stringify(reportList))
        elem.style.width = '100%'
        elem.style.padding = '3px !important'
        //elem._updateLayout()
      }
    })
    ...
    // function afterUI () {
    //   // reports needs initialization
    //   const elem = document.getElementById(self.props.id + '_' + 'reports1')
    //   if (elem) {
    //     // @ts-expect-error
    //     elem.initialize(self.props.user.username, self.props.token, self.state.sourceCountry, self.state.langCode)
    //     elem.style.width = '100%'
    //     elem.style.padding = '3px !important'
    //   }
    // }
  ...
  onSearchTypeChanged = (searchType, checked) => {
    switch (searchType) {
      ...
      case ('geographies'):{
        ...
        this.onPropertyChange('geographies', checked)
        const delay = () => { this.onPropertyChange('noGeographySearch', (checked) ? 'false' : 'true') }
        setTimeout(delay, 0)
        break
      }
    }
  ...
  render () {
    const { selectedCountry, activeGeographyLevels, geographiesChecked, presetBuffersQueued, maxDriveBuffer, maxWalkBuffer } = this.state
    const { reportList, geographies, availableGeographyLevels, selectedGeographyLevels } = this.props.config
    const { widgetMode, viewMode, backgroundColor, runReportOnClick, imageExport, dynamicHtml, excel, pdf, print, fullscreen, zoomLevel, displayHeader, headerColor, headerTextColor, sourceCountry } = this.props.config
        ...
        <React.Fragment>
          ...
          >
            ...
            >
              ...
              <Button className='float-right' type='primary' size='sm' href='https://links.esri.com/ba-exb/modes' target='_blank'>
        ...
        <div className='widget-setting-bao' style={{ display: 'relative' }}>
                          ...
                          <React.Fragment>
                            ...
                            <SettingRow>
                              <Label check centric>
                                ...
                                <Checkbox style={{ cursor: 'pointer' }} className='mr-2' checked disabled />
                                {this.localeString('pointsOfInterest')}
                                {/* <Checkbox style={{ cursor: 'pointer' }} className='mr-2' checked={pointsOfInterest} onChange={e => {
                                    this.onSearchTypeChanged('pointsOfInterest', e.target.checked)
                                    this.onPropertyChange('pointsOfInterest', e.target.checked)
                                  }} /> */}
                            ...
                            {/* GEOGRAPHY LEVELS */}
                            {geographies && availableGeographyLevels &&
                                ...
                                <SettingRow>
                                  <Label check centric>
                                    <Checkbox style={{ cursor: 'pointer' }} checked={isSelected}
                                      onChange={e => {
                                        this.updateGeographyLevels({ level }, e.target.checked)
                                      }} className='ml-3 mr-2' />
                                    {level}
                                  </Label>
                                </SettingRow>
            ...
            ? <SettingSection title={<div className='w-100 d-flex' style={{ height: '23px', flexFlow: 'column wrap', alignContent: 'space-between' }}>
                ...
                <SettingRow flow='wrap'>
                  {/* <div className='w-100' ref={this.sidePopperTrigger}>
                    <Button type='primary' onClick={() => this.toggleSidePopper('presetBufferSidePopper')} style={{ width: '100%' }}>{this.localeString('setBuffers')}</Button>
                  </div> */}
              ...
              {workflowInfographicSidePopper && (
                <SidePopper isOpen title={widgetMode === Mode.Workflow ? this.localeString('customizeInfographicsBtn') : this.localeString('selectAnInfographic')} position='right' toggle={() => this.toggleSidePopper('workflowInfographicSidePopper')} trigger={this.sidePopperTrigger?.current}>
                  ...
                  <div className='p-3' style={popperOuterStyle}>
                      ...
                      <Switch className='can-x-switch' data-key='workflowEnableInfographicChoice' checked={workflowEnableInfographicChoice} onChange={e => { this.onPropertyChange('workflowEnableInfographicChoice', e.target.checked) }} />
                    </SettingRow>
          ...
          >
            <SettingRow flow='no-wrap' className='mt-3' label={this.localeString('viewMode')}>
              <Select className='w-50' name='viewMode' size='sm' value={viewMode} onChange={e => { this.handleIgSettingChange('viewMode', e.target.value) }}>
                ...
                <option key={ViewMode.StackAll}
                  value={ViewMode.StackAll}>{this.localeString('panelsInStackAll')}</option>
              </Select>
            </SettingRow>
            <SettingRow label={this.localeString('backgroundColor')}>
              <ColorPicker
                  ...
                  onChange={value => {
                    this.handleIgSettingChange('backgroundColor', value)
                  }}
            ...
            {displayHeader && (
              <React.Fragment>
                ...
                <SettingRow label={this.localeString('printButton')}>
                  <Switch className='can-x-switch' data-key='print' checked={print}
                    onChange={e => {
                      this.handleIgSettingChange('print', e.target.checked)
                    }} />
                </SettingRow>
