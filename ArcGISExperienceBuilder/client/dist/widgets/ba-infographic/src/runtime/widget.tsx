...
import { React, AllWidgetProps, getAppStore, jsx, SessionManager, IMState, UseUtility, UtilityManager } from 'jimu-core'
import { ArcgisInfographic, ArcgisBaSearch, ArcgisInfographicTree } from '../../node_modules/@arcgis/business-analyst-components/dist/components'
...
import { WidgetPlaceholder, NumericInput, Select, Icon, Row, Container, Tooltip, Modal, ModalBody, ModalHeader } from 'jimu-ui'
...
import Logz from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/assets/ba-reports/esm/Logz'
...
any> {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      noWorkflowGeographySearch: typeof props.config.noGeometrySearch !== 'undefined' ? props.config.noGeometrySearch : false,
  ...
  setMaxBuffers (bufferType, bufferUnit = null) {
    ...
    if ((bufferType === InfoBufferType.drivetime) && ACLUtils.isDef(useUnit)) {
      if (useUnit === 'minutes') {
        this.updateState('maxDriveBuffer', MaxBuffers.DriveMinutes, this.enforceMax(InfoBufferType.drivetime, MaxBuffers.DriveMinutes))
      } else if (useUnit === 'miles') {
        this.updateState('maxDriveBuffer', MaxBuffers.DriveMile, this.enforceMax(InfoBufferType.drivetime, MaxBuffers.DriveMile))
      } else if (useUnit === 'kilometers') {
        this.updateState('maxDriveBuffer', MaxBuffers.DriveKm, this.enforceMax(InfoBufferType.drivetime, MaxBuffers.DriveKm))
      }
    }
    if ((bufferType === InfoBufferType.walktime) && ACLUtils.isDef(useUnit)) {
      if (useUnit === 'minutes') {
        this.updateState('maxWalkBuffer', MaxBuffers.WalkMinutes, this.enforceMax(InfoBufferType.walktime, MaxBuffers.WalkMinutes))
      } else if (useUnit === 'miles') {
        this.updateState('maxWalkBuffer', MaxBuffers.WalkMile, this.enforceMax(InfoBufferType.walktime, MaxBuffers.WalkMile))
      } else if (useUnit === 'kilometers') {
        this.updateState('maxWalkBuffer', MaxBuffers.WalkKm, this.enforceMax(InfoBufferType.walktime, MaxBuffers.WalkKm))
      }
    }
  }
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    const { widgetMode, sourceCountry, noGeographySearch, runReportOnClick, reportList, backgroundColor } = self.props.config
    ...
    const { stencilPropChangeConnected, noWorkflowGeographySearch, selectedCountry, showInfographicModal, currentStep, workflowRuntimeSearchSelectedObject, workflowRuntimeSelectedFeatureObjectFirstTime, workflowRuntimeBuffer } = self.state
    ...
    if (noWorkflowGeographySearch !== noGeographySearch) {
      self.updateState('noWorkflowGeographySearch', noGeographySearch)
    }
    ...
    if (self.getBackgroundColorFromProps || prevProps.config.backgroundColor !== backgroundColor) {
      self.getBackgroundColorFromProps = false
      self.updateState('spinnerBackgroundColor', backgroundColor)
    }

    if (prevProps.config.runReportOnClick !== runReportOnClick && self.mapActions) {
      self.mapActions.allowMapClicks(runReportOnClick)
    }
    ...
    if (prevProps.config.noGeographySearch !== noGeographySearch) {
      self.updateState('noWorkflowGeographySearch', noGeographySearch)
    }
  ...
  setHeightInfographicTree () {
    ...
    const widgetElem = document.querySelector('div.widget-infographic-player')
    const introReportsRowElem = document.getElementById('workflowIntroTextReportsRow')
  ...
  onSiteObjectChanged (result: any) {
    ...
    switch (result.origin) {
      ...
      case ('basearch'): {
        ...
        context.updateState('workflowRuntimeSearchSelectedObject', JSON.stringify(searchObject))
        break
      }
      case ('settingsPreset'): {
  ...
  activeViewChangeHandler (jmv: JimuMapView, context: any) {
    ...
    if (jmv && jmv.view) {
      jmv.view.when(function (event) {
        ...
        self.mapActions.allowMapClicks(self.props.config.runReportOnClick)
  ...
  buildInfographicOptions (provideResult?: boolean) {
    const { widgetMode, viewMode, displayHeader, zoomLevel, fullscreen, excel, imageExport, print, pdf, dynamicHtml, backgroundColor, headerColor, headerTextColor, runReportOnClick } = this.props.config
  ...
  validateInfographicData (announce?: boolean) {
    ...
    const { user, token } = this.props
    const { presetSelectedReport, widgetMode, workflowSearchSelectedObject, workflowSelectedReport, presetSearchSelectedObject } = this.props.config
    let logged = false
  ...
  render () {
    ...
    let { workflowIntroText, workflowIntroTextBuffers, workflowIntroTextReports } = this.props.config

    if (!workflowIntroText) {
      workflowIntroText = this.localeString('searchIntro')
    }

    if (!workflowIntroTextBuffers) {
      workflowIntroTextBuffers = this.localeString('bufferExtentDesc')
    }

    if (!workflowIntroTextReports) {
      workflowIntroTextReports = this.localeString('infographicIntro')
    }
    ...
    return (
      <div id={ this.widgetOuterDivId }
            ...
            <React.Fragment>
              <div className='containerStyle'>
                ...
                <Container className='contentStyle'>
                  <Row>
                    <CalciteStepper id={this.workflowStepperId} layout='horizontal' scale='s'>
                      {/*-----Step #1 --------------------------------------------------------*/}

                      {this.stepIsVisible(Steps.Search) &&
                          <CalciteStepperItem id={this.workflowStepperId + '-1'} {...searchTabStatus} heading={this.localeString('search')} tab='search' >

                            <Container className="stepContentContainerStyle">
                              <Row className="stepContentStyle">
                                  {/*Step 1 CONTENT HERE*/}
                                  <Container>
                                    {workflowDisplayIntroText &&
                                      <Row className="mb-2">
                                    ...
                                    <div style={{ position: 'relative' }}>
                                      <Row style={searchParentRow}>
                                        <ArcgisBaSearch
                                          id={this.workflowRuntimeSearchId}
                                          ...
                                          langCode={langCode}
                                          suglite='true'
                                          maxsuggestionheight={suggestionHeight}
                                          activeLevels={JSON.stringify(selectedGeographyLevels)}
                                        />

                                      </Row>
                                      {/* Selected Search Result Over Div */}
                                      {searchObj != null &&
                                        <Row className='mt-2 selectedSearchResultRowWF'>
                                          <div className='d-flex selectedSearchResultWF'>
                                              <div className='d-flex' style={{ flex: '0 1 auto', alignItems: 'center', marginRight: '7px' }}>
                                          <Icon size='l' icon={this.isLocationType(searchObj.type) ? PinEsriOutlined : PolygonOutlined} />
                                            </div>
                                            <div className='d-flex' style={{ flex: '1 1 auto', textAlign: 'left', justifyContent: 'center', flexDirection: 'column' }}>
                                              {this.searchResultString}
                                            </div>
                                            <Tooltip style={{ margin: '10px' }} placement='bottom-start' title={this.localeString('clearSearch')}>
                                              <div className='d-flex' style={{ margin: 'auto 0', alignItems: 'right', cursor: 'pointer' }} onClick={() => { onClickSearchResultOverDiv() }}>
                      ...
                      {/*-----Step #2 --------------------------------------------------------*/}
                        ...
                        {this.stepIsVisible(Steps.Buffers) &&
                          <CalciteStepperItem id={this.workflowStepperId + '-2'} {...bufferTabStatus} heading={this.localeString('setBuffers')} tab='buffers' >
                            <Container className="stepContentContainerStyle">
                              <Row className="stepContentStyle">
                                <Container>
                                  {workflowIntroTextBuffersCheckbox &&
                                  ...
                                  <Row className="mb-2">
                                    <CalciteTabs>
                                      <CalciteTabNav slot="title-group" id={'buffers-nav-' + this.props.id}>
                                          {workflowAvailableBufferRings &&
                                        <CalciteTabTitle id={'tab-title-rings-' + this.props.id} onCalciteTabsActivate={(e) => { this.updateWorkflowBufferState('workflowRuntimeBuffer', InfoBufferType.ring) }}>{this.localeString('rings')}</CalciteTabTitle>
                                          }
                                          {workflowAvailableBufferDrivetime &&
                                            <CalciteTabTitle id={'tab-title-drive-' + this.props.id} onCalciteTabsActivate={(e) => { this.updateWorkflowBufferState('workflowRuntimeBuffer', InfoBufferType.drivetime) }}>{this.localeString(InfoBufferType.drivetime)}</CalciteTabTitle>
                                          }
                                          {workflowAvailableBufferWalktime &&
                                            <CalciteTabTitle id={'tab-title-walk-' + this.props.id} onCalciteTabsActivate={(e) => { this.updateWorkflowBufferState('workflowRuntimeBuffer', InfoBufferType.walktime) }}>{this.localeString(InfoBufferType.walktime)}</CalciteTabTitle>
                                          }
                                      </CalciteTabNav>
                                      {workflowAvailableBufferRings &&
                                        <CalciteTab id={'tab-rings-' + id}>
                                          <Row flow='no-wrap' className='w-100 d-flex pt-3' style={{ margin: '0px' }}>
                                            <div className='pr-2' style={{ display: 'flex', alignItems: 'center' }}>{this.localeString('radius')}</div>
                                            <NumericInput min='0.1' max={MaxBuffers.Rings} showHandlers={false} name='workflowRuntimeRingsBuffer1' data-key='workflowRuntimeRingsBuffer1' className='bufferInput' size='sm' value={workflowRuntimeRingsBuffer1} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeRingsBuffer1', buffer, InfoBufferType.ring) }}/>
                                            <NumericInput min='0.1' max={MaxBuffers.Rings} showHandlers={false} name='workflowRuntimeRingsBuffer2' data-key='workflowRuntimeRingsBuffer2' className='bufferInput mx-1' size='sm' value={workflowRuntimeRingsBuffer2} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeRingsBuffer2', buffer, InfoBufferType.ring) }} />
                                            <NumericInput min='0.1' max={MaxBuffers.Rings} showHandlers={false} name='workflowRuntimeRingsBuffer3' data-key='workflowRuntimeRingsBuffer3' className='bufferInput' size='sm' value={workflowRuntimeRingsBuffer3} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeRingsBuffer3', buffer, InfoBufferType.ring) }} />
                                            <Select name='workflowRuntimeRingsBufferUnit' className='bufferUnits ml-1' size='sm' value={workflowRuntimeRingsBufferUnit} onChange={(e) => { this.handleWorkflowBufferChange('workflowRuntimeRingsBufferUnit', e.currentTarget.value, InfoBufferType.ring) }}>
                                              <option value='miles'>{this.localeString('milesLow')}</option>
                                              <option value='kilometers'>{this.localeString('kilometersLow')}</option>
                                            </Select>
                                          </Row>
                                        </CalciteTab>
                                      }
                                      {workflowAvailableBufferDrivetime &&
                                        <CalciteTab id={'tab-drive-' + id}>
                                          <Row flow='no-wrap' className='w-100 d-flex pt-3' style={{ margin: '0px' }}>
                                            <div className='pr-2' style={{ display: 'flex', alignItems: 'center' }}>{this.localeString('time')}</div>
                                            <NumericInput min='1' max={maxDriveBuffer} showHandlers={false} name='workflowRuntimeDrivetimeBuffer1' data-key='workflowRuntimeDrivetimeBuffer2' className='bufferInput' size='sm' value={workflowRuntimeDrivetimeBuffer1} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeDrivetimeBuffer1', buffer, InfoBufferType.drivetime) }}/>
                                            <NumericInput min='1' max={maxDriveBuffer} showHandlers={false} name='workflowRuntimeDrivetimeBuffer2' data-key='workflowRuntimeDrivetimeBuffer2' className='bufferInput mx-1' size='sm' value={workflowRuntimeDrivetimeBuffer2} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeDrivetimeBuffer2', buffer, InfoBufferType.drivetime) }} />
                                            <NumericInput min='1' max={maxDriveBuffer} showHandlers={false} name='workflowRuntimeDrivetimeBuffer3' data-key='workflowRuntimeDrivetimeBuffer3' className='bufferInput' size='sm' value={workflowRuntimeDrivetimeBuffer3} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeDrivetimeBuffer3', buffer, InfoBufferType.drivetime) }} />
                                            <Select name='workflowRuntimeDrivetimeBufferUnit' className='bufferUnits ml-1' size='sm' value={workflowRuntimeDrivetimeBufferUnit} onChange={(e) => { this.handleWorkflowBufferChange('workflowRuntimeDrivetimeBufferUnit', e.currentTarget.value, InfoBufferType.drivetime) }}>
                                      ...
                                      {workflowAvailableBufferWalktime &&
                                        <CalciteTab id={'tab-walk-' + id}>
                                          <Row flow='no-wrap' className='w-100 d-flex pt-3' style={{ margin: '0px' }}>
                                            <div className='pr-2' style={{ display: 'flex', alignItems: 'center' }}>{this.localeString('time')}</div>
                                            <NumericInput min='1' max={maxWalkBuffer} showHandlers={false} name='workflowRuntimeWalktimeBuffer1' data-key='workflowRuntimeWalktimeBuffer1' className='bufferInput' size='sm' value={workflowRuntimeWalktimeBuffer1} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeWalktimeBuffer1', buffer, InfoBufferType.walktime) }} />
                                            <NumericInput min='1' max={maxWalkBuffer} showHandlers={false} name='workflowRuntimeWalktimeBuffer2' data-key='workflowRuntimeWalktimeBuffer2' className='bufferInput mx-1' size='sm' value={workflowRuntimeWalktimeBuffer2} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeWalktimeBuffer2', buffer, InfoBufferType.walktime) }} />
                                            <NumericInput min='1' max={maxWalkBuffer} showHandlers={false} name='workflowRuntimeWalktimeBuffer3' data-key='workflowRuntimeWalktimeBuffer3' className='bufferInput' size='sm' value={workflowRuntimeWalktimeBuffer3} onChange={(buffer) => { this.handleWorkflowBufferChange('workflowRuntimeWalktimeBuffer3', buffer, InfoBufferType.walktime) }} />
                                            <Select name='workflowRuntimeWalktimeBufferUnit' className='bufferUnits ml-1' size='sm' value={workflowRuntimeWalktimeBufferUnit} onChange={(e) => { this.handleWorkflowBufferChange('workflowRuntimeWalktimeBufferUnit', e.currentTarget.value, InfoBufferType.walktime) }}>
                                              <option value='minutes'>{this.localeString('minuteLow')}</option>
                                              <option value='miles'>{this.localeString('milesLow')}</option>
                                              <option value='kilometers'>{this.localeString('kilometersLow')}</option>
                                            </Select>
                      ...
                      {this.stepIsVisible(Steps.Infographic) &&
                        <CalciteStepperItem id={this.workflowStepperId + '-3'} {...infographicTabStatus} heading={this.localeString('selectInfographic')} tab='infographic' onClick={() => this.updateState('infographicTabVisited', true)}>
                          <Container className="stepContentContainerStyle">
                            <Row id={this.workflowRuntimeSearchRowId} className="stepContentStyle">
                              <Container>
                                {workflowIntroTextReportCheckbox &&
                                  <Row className="mb-2" id="workflowIntroTextReportsRow">
                                    {workflowIntroTextReports}
                                  </Row>
                                }

                                <Row className="mb-2">
                                  <ArcgisInfographicTree
                                    id={this.workflowReportListId}
                                    style={reportListStyle}
                                    mode={widgetMode}
                                    env={window.jimuConfig.hostEnv}
                                    username={user.username}
                                    token={token}
                                    ...
                                    sourceCountry={selectedCountry}
                                    langCode={langCode}
                                    selectedReportId={workflowReportId}
                ...
                <Container className='navStyle'>
                  <Row className='navDivStyle'>
                    {this.navButtonIsVisible(NavButtons.Previous) &&
                      <CalciteButton className='navPrevButtonStyle' {...this.navButtonAppearance(NavButtons.Previous)} scale="s" onClick={() => this.stepGoToVisible('prev')}>
                        {this.localeString('back')}
                      </CalciteButton>
                    }
                    {this.navButtonIsVisible(NavButtons.Next) &&
                      <CalciteButton className='navNextButtonStyle' {...this.navButtonAppearance(NavButtons.Next)} {...nextButtonStatus} scale="s" onClick={() => this.stepGoToVisible('next')}>
                        {this.localeString('next')}
                      </CalciteButton>
                    }
                    {this.navButtonIsVisible(NavButtons.Infographic) &&
                      <CalciteButton className='navRunInfographicStyle' {...this.navButtonAppearance(NavButtons.Infographic)} scale="s" onClick={() => this.workflowRunInfographic()}>
                        {this.localeString('runInfographic')}
                      </CalciteButton>
                    }
                  </Row>
                </Container>
        ...
        {(showInfographicModal === true && igData !== undefined) && (
          <Modal className='d-flex justify-content-between' isOpen={showInfographicModal} role='dialog' style={{ width: '100vw', height: '100vh', maxWidth: 'unset', margin: '0 auto' }}>
            <ModalHeader toggle={() => this.updateState('showInfographicModal', false)}></ModalHeader>
            <ModalBody>
              <div slot="content" style={{ padding: '0px', height: '100%', backgroundColor: mainBackgroundColor }}>
                <img src={require('./assets/largeBusy.gif')} style={mainBackgroundSpinner}></img>

                <ArcgisInfographic
                  id={widgetMode === Mode.Preset ? this.presetInfographicId : this.workflowModalInfographicId}
                  style={modalInfographic}
                  instanceId={id}
                  env={igData.env}
                  username={igData.username}
                  token={igData.token}
                  ...
                  theme={this.stringifyTheme()}
                  locationName={igData.locationName}
                  locationAttributes={igData.attributes}
                  sourceCountry={igData.country}
                  options={JSON.stringify(igData.buffers.infographicOptions)}
                  langCode={langCode}
                  reportId={igData.report}
