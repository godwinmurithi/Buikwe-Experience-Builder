...
import TransportUtil from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/assets/ba-reports/esm/mobile/TransportUtil'
import Environments from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/assets/ba-reports/esm/Environments'
import Debounce from '../utils/debounce'
import GeocoderClient from '../../node_modules/@arcgis/business-analyst-components/dist/stencil-components/dist/assets/ba-reports/esm/mobile/GeocoderClient.js'
/**
 *  Map actions:
...
export default class BAMapActions {
  ...
  /** constructor
     ...
     * }
     */
  constructor (widgetId: string, mapId: string, showSearch: boolean, env: string, reportChangeCallback: any, context, searchPlaceholderLocalized: string) {
            ...
            function (response) {
              ...
              if ((response.results.length <= 0) || (!getGraphics(response))) {
                if (self.context && self.context.onSiteObjectChanged) {
                  ...
                  try {
                    Environments.setEnvironment(self.env)
                    const g = new GeocoderClient(creds[1], null)
   ...
   */
  public renderBoundary (data: any) {
   ...
   */
  public renderLocation (data: any) {
    const self = this
    if (this._initialized) {
      ...
      this._renderToMap(true, _drawMarker)
    } else {
      this._warnNotInitialized()
    }
  }
   ...
   * ----------------------------------------------------------------------------------
   */
  private _renderToMap (noClear?: boolean, markerRenderFunc?: any) {
    const self = this
    // this.syncSettings()
    if (!noClear) {
      // this.restartGraphics()
