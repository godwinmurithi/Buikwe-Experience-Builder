import { React, DataSource, DataSourceConstructorOptions, DataSourceManager, DataSourcesChangeMessage, DataSourcesChangeType, Immutable, loadArcGISJSAPIModules, MessageManager, MutableStoreManager, ServiceDefinition, DataSourceJson } from 'jimu-core'
import { DataSourceTypes, FeatureLayerDataSourceConstructorOptions } from 'jimu-core/data-source'
...
export async function createDataSourcesByDataOptions (multiDataOptions: DataOptions[], widgetId: string, publishMessage = true): Promise<DataSource[]> {
  ...
  return Promise.allSettled(dataSourceConstructorOptions.map(o => DataSourceManager.getInstance().createDataSource(o)))
