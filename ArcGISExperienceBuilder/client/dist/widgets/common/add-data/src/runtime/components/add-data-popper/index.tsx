/** @jsx jsx */
import { React, jsx, css, defaultMessages as jimuCoreMessages } from 'jimu-core'
import { hooks, defaultMessages as jimuUIMessages, Button, Popper, Tab, Tabs, PanelHeader, Alert, MobilePanel } from 'jimu-ui'
...
export const AddDataPopper = (props: AddDataPopperProps) => {
  const { portalUrl, widgetId, buttonSize, hiddenTabs, popperReference, nextOrder: propsNextOrder, onFinish: propsOnFinish } = props
  ...
  const translate = hooks.useTranslate(jimuUIMessages, jimuCoreMessages, defaultMessages)
  ...
  return <div className='add-data-popper' css={style}>
    ...
    {
      buttonSize === 'sm' &&
      <Button type='primary' className='d-flex justify-content-center align-items-center small-add-btn' onClick={togglePopper} aria-label={translate('addData')}>
        <PlusOutlined size='m' className='m-0' />
      </Button>
    }
    {
      mobile
        ? <MobilePanel open={isOpen} toggle={togglePopper} title={translate('addData')}>
        {getPopperContent()}
      </MobilePanel>
        : <Popper open={isOpen} toggle={togglePopper} reference={popperReference} placement='right-start'>
          {getPopperContent()}
        </Popper>
    }
  </div>
...
const TabContent = ({ tab, portalUrl, widgetId, nextOrder, multiDataOptionsFromSearch, multiDataOptionsFromUrl, multiDataOptionsFromFile, setMultiDataOptionsFromSearch, setMultiDataOptionsFromUrl, setMultiDataOptionsFromFile, setErrorMsg }: { tab: SupportedTabs, portalUrl: string, widgetId: string, nextOrder: number, multiDataOptionsFromSearch: DataOptions[], multiDataOptionsFromUrl: DataOptions[], multiDataOptionsFromFile: DataOptions[], setMultiDataOptionsFromSearch: (multiDataOptions: DataOptions[]) => void, setMultiDataOptionsFromUrl: (multiDataOptions: DataOptions[]) => void, setMultiDataOptionsFromFile: (multiDataOptions: DataOptions[]) => void, setErrorMsg: (msg: string) => void }) => {
  if (tab === 'search') {
    return <DataItemSearch portalUrl={portalUrl} widgetId={widgetId} onChange={setMultiDataOptionsFromSearch} nextOrder={nextOrder} multiDataOptions={multiDataOptionsFromSearch} />
  } else if (tab === 'url') {
