/** @jsx jsx */
import { React, jsx, css, uuidv1, DataSourceTypes, loadArcGISJSAPIModule, getAppStore, defaultMessages as jimuCoreMessages } from 'jimu-core'
import { Loading, LoadingType, Input, Label, hooks, Icon, Button } from 'jimu-ui'

import { PlusOutlined } from 'jimu-icons/outlined/editor/plus'

import defaultMessages from '../../translations/default'
import { DataOptions, FeatureCollection, LayerInFeatureCollection } from '../../types'
...
export const DataFileUpload = (props: DataFileUploadProps) => {
  ...
  const translate = hooks.useTranslate(jimuCoreMessages, defaultMessages)
  ...
  return <div className='data-file-upload w-100 h-100 pb-3 pt-5 px-3' css={style}>
    <div className='supported-type-icons d-flex justify-content-around align-items-center px-5 mb-3'>
      <Icon width={25} height={31} icon={require('../../assets/file1.svg')} />
      <Icon width={42} height={42} icon={require('../../assets/file2.svg')} />
      <Icon width={25} height={31} icon={require('../../assets/file3.svg')} />
    </div>
    ...
    <div className='mt-3 drag-area-container'>
      <Label for={dragToUploadBtnId} className='drag-area text-center'>
        ...
        <div className='upload-btn-container w-75' title={translate('upload')}>
          <Label for={clickToUploadBtnId} className='upload-btn text-center mt-3 mb-0 text-truncate'>
            <PlusOutlined size={15} className='mr-2' />
            <span>{translate('upload')}</span>
          </Label>
          <Input id={clickToUploadBtnId} title='' className='upload-btn-file-input' type='file' accept={INPUT_ACCEPT} onChange={onFileChange} />
        </div>
      </Label>
      <Input id={dragToUploadBtnId} onClick={preventDefault} title='' className='drag-area-file-input' type='file' accept={INPUT_ACCEPT} onChange={onFileChange} />
    </div>
