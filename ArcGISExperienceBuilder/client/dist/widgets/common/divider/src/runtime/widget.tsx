...
> {
  ...
  getDividerLinePositionStyle = (config: Config) => {
    ...
    const pointStartSize =
      pointStart.pointSize * this.getSize(strokeStyle?.size)
  ...
  toggleQuickStyle = (isOpen = false) => {
    this.props.dispatch(
      appActions.widgetStatePropChange(this.props.id, 'showQuickStyle', isOpen)
    )
  }
  ...
  getQuickStyleComponent = () => {
    ...
    const QuickStyle = this?.props?.builderSupportModules?.widgetModules
      ?.QuickStyle
    return (!QuickStyle && isMount)
      ...
      : (
        <QuickStyle
          ...
          onInitDragHandler={onInitDragHandler} onInitResizeHandler={onInitResizeHandler}
        />
        )
  }
