...
const belongtoDataSourceKey = 'table-usedDs-info'
...
> {
  ...
  static mapExtraStateProps = (
    ...
    dsIds?.forEach(dsId => {
      ...
      belongToDataSourceInfos[`${belongtoDataSourceKey}_${dsId}`] = state?.dataSourcesInfo?.[belongToDs?.id]
    })
  ...
  componentDidUpdate (prevProps: AllWidgetProps<IMConfig> & Props, prevState: State) {
    ...
    const preDsId = `${belongtoDataSourceKey}_${prevCurConfig?.useDataSource?.dataSourceId}`
    const curDsId = `${belongtoDataSourceKey}_${newCurConfig?.useDataSource?.dataSourceId}`
    const preBelongToWidgetQuery = prevProps?.[preDsId]?.widgetQueries
    const curBelongToWidgetQuery = this.props?.[curDsId]?.widgetQueries
    const preBelongToSourceVersion = prevProps?.[preDsId]?.sourceVersion
    const curBelongToSourceVersion = this.props?.[curDsId]?.sourceVersion
    const curBelongToDsStatus = this.props?.[curDsId]?.status
  ...
  getFieldsFromDatasource = () => {
    ...
    // if dataActionDataSource exists, use it
  ...
  createTable = (newDataSource?, versionChange?: boolean) => {
    ...
    const { dataActionDataSource } = curLayer
    ...
    const curDsId = dataActionDataSource ? dataActionDataSource?.id : curLayer?.useDataSource?.dataSourceId
    ...
    const getLayerAndNewTable = () => {
      ...
      if (dataSourceUsed) {
        const tablePromise = cancelablePromise.cancelable(this.getFeatureLayer(dataSourceUsed, dataRecords).then(async layer => {
          ...
          if (!this.refs.tableContainer || !document) {
            return
          }
