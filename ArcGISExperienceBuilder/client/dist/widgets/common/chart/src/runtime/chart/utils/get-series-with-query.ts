...
const getSeriesWithQuery = (series: ImmutableArray<WebChartSeries>, query: ImmutableObject<FeatureLayerQueryParams>, setQueryForPie = false) => {
  let callback = null
  const type = getSeriesType(series as any)
  // don't set query for pie chart(/arcgis-charts/issues/5355)
  if (isSerialSeries(type) || (type === 'pieSeries' && setQueryForPie)) {
