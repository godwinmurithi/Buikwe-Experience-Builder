...
const ChartTypeSelector = (props: ChartTypeSelectorProps): React.ReactElement => {
  const { templateId, webChart, useDataSources, 'aria-label': ariaLabel, onChange } = props
  ...
  return (
    <React.Fragment>
      <div className="chart-type-selector w-100" css={style}>
        {!templateId && <Button aria-label={ariaLabel} ref={ref} className='w-100' type='link' onClick={() => setOpen(v => !v)}>
