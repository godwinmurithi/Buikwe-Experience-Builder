...
export const AxesSetting = (props: AxesSettingProps): React.ReactElement => {
  const { showLogarithmicScale, showFirstValueRange = true, axes: propAxes, onChange, rotated } = props
      ...
      {propAxes?.map((axis, index) => {
        ...
        return (
          ...
          >
            ...
            {
              type === 'number' && (
                <NumericAxis className='mt-3' showValueRange={index === 0 ? showFirstValueRange : true} showLogarithmicScale={showLogarithmicScale} isHorizontal={isHorizontal} axis={axis} onChange={handleChange} />
              )
            }
            {
              type === 'date' && (
