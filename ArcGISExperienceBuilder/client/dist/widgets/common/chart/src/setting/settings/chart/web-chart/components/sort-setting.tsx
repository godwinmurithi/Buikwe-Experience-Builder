...
import { Button, hooks, Select } from 'jimu-ui'
...
export const SorteSetting = (props: SorteSettingProps): React.ReactElement => {
  const { value = ' ASC', fields = Immutable([]), 'aria-label': ariaLabel, onChange } = props
...
export const OrderArrow = ({ value = 'ASC', onChange }: OrderArrowProps): React.ReactElement => {
  ...
  return (
    <Button title={label} aria-label={label} type='tertiary' className='order-arrow' size='sm' icon onClick={handleClick}>
