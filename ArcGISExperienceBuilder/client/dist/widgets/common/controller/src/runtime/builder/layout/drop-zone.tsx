...
export const DrapZone = React.forwardRef((props: DropZoneProps, ref) => {
  ...
  const onDragOver = (_, __, containerRect: Partial<Partial<DOMRectReadOnly>>, itemRect: Partial<Partial<DOMRectReadOnly>>) => {
    let rect = itemRect
    ...
    if (childRects && childRects.length) {
      if (!vertical) {
        const { insertX, refId } = calInsertPositionForRow(containerRect as Partial<DOMRectReadOnly>, rect, childRects)
        ...
        const { insertY, refId } = calInsertPositionForColumn(containerRect as Partial<DOMRectReadOnly>, rect, childRects)
        referenceIdRef.current = refId
        rect = {
