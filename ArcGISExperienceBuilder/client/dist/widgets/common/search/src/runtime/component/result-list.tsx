/** @jsx jsx */
import { React, jsx, QueriableDataSource, classNames, DataRecord, Immutable, ImmutableObject, i18n, MessageManager, DataRecordsSelectionChangeMessage, RecordSetChangeType } from 'jimu-core'
import { hooks, Icon, Button, DropdownItem } from 'jimu-ui'
import { IMConfig, IMServiceList, SearchServiceType } from '../../config'
import defaultMessage from '../translations/default'
import { getDatasourceConfigItemByConfigId, getJsonLength, getDatasource, loadAllDsRecord, publishRecordCreatedMessageAction } from '../utils/utils'
...
const ResultList = (props: ResultListProps) => {
  ...
  const { reference, searchText, id, config, serviceList, isOpentResultListDefault, searchInputRef, setResultFirstItem, handleDsIdOfSelectedResultItemChange } = props
        ...
        <div key={`${configId}_${label}_con`} role='group' title='label' aria-label={label}>
          {displayItem.length > 0 && <div>
            {currentOutputNumber > 1 &&
              <Button role='button' className='source-label-con' disabled={true} key={`${configId}_${label}`} title={label}>
  ...
  return (
    <div role='group' aria-label={nls('searchResults')}>
      <ResultPopper
        ...
        id={id}
        className='result-list-con'
      >
        <div className='result-list-con'>
          <div ref={dropdownMenuRef}>
            {!isNoResult() && <Button role='button' className='d-flex align-items-center show-result-button' onClick={onShowResultButtonclick} title={nls('searchResults')}>
