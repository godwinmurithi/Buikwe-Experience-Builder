...
import { IMConfig, SuggestionItem, Suggestion, IMSearchDataConfig, DatasourceListItem, RecordResultType } from '../../config'
...
export const loadDsRecords = (serviceListItem: DatasourceListItem, resultMaxNumber: number, id: string): Promise<RecordResultType> => {
  ...
  if (!checkIsDsCreated(dsId)) return Promise.resolve({} as RecordResultType)
