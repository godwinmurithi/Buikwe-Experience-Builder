import { DataSource, DataSourceManager, DataSourceStatus, QueriableDataSource, MessageManager, DataRecordSetChangeMessage, RecordSetChangeType, urlUtils, utils } from 'jimu-core'
import { RECENT_SEARCHES_KEY, IMConfig, SearchDataConfig, SuggestionItem, ServiceList, SearchServiceType, Suggestion, SearchResultView, RecordResultType } from '../../config'
...
export const clearFilterOfDeletedDs = (dsId: string, widgetId: string) => {
  const useDataSource = getDatasource(dsId)
  //Update datasource query params
  useDataSource && (useDataSource as QueriableDataSource).updateQueryParams({}, widgetId)
}
...
export const setRecentSearches = (options: SetRecentSearcheOptions) => {
  ...
  const recentSearchKey = `exb-${appId}-${id}_${RECENT_SEARCHES_KEY}`
...
export const getRecentSearches = (id: string): string[] => {
  const appId = urlUtils.getAppIdPageIdFromUrl().appId
  const recentSearchKey = `exb-${appId}-${id}_${RECENT_SEARCHES_KEY}`
  const oldRecentSearchKey = `${id}_${RECENT_SEARCHES_KEY}`
  let recentSearchInLocal = utils.readLocalStorage(recentSearchKey) || utils.readLocalStorage(oldRecentSearchKey)
...
export const clearRecentSearches = (id: string) => {
  ...
  const recentSearchKey = `exb-${appId}-${id}_${RECENT_SEARCHES_KEY}`
  const oldRecentSearchKey = `${id}_${RECENT_SEARCHES_KEY}`
  const oldRecentSearchInLocal = utils.readLocalStorage(oldRecentSearchKey)
  utils.setLocalStorage(recentSearchKey, '')
  oldRecentSearchInLocal && utils.setLocalStorage(oldRecentSearchKey, '')
}
