...
import { getDatasource, setRecentSearches, getRecentSearches, clearFilterOfDeletedDs, clearRecentSearches, getJsonLength, changeDsStatus, checkIsDsCreated, getSuggestions, updateAllDsQueryParams, loadAllDsRecord, publishRecordCreatedMessageAction } from '../utils/utils'
      ...
      clearSearchValueTimeoutRef.current = setTimeout(() => {
        ...
        deleteConfigIds?.forEach(configId => {
          ...
          if (isItemFeatureService) {
            clearFilterOfDeletedDs(preService[configId].useDataSource?.dataSourceId, id)
          }
        })
      })
    }
  ...
  const initOutputDsStatus = hooks.useEventCallback(() => {
    for (const configId in serviceList) {
      if (serviceList[configId]?.searchServiceType === SearchServiceType.GeocodeService) {
        ...
        publishRecordCreatedMessageAction(id, [], RecordSetChangeType.Remove)
      }
    }
  })
  ...
  */
  ...
  const inputConKeyup = (e) => {
    if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
      isFocusSuggestion.current = true
    }
    checkAndFocksPopper(e)
  }
