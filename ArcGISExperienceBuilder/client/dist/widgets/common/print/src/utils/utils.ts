...
export function getPrintTemplateTaskInfo (printServiceUrl: string): Promise<any[]> {
  ...
  return getPrintTemplateTaskInfoUrl(printServiceUrl).then(urInfo => {
    const { url, executionType, templatesInfoTaskUrl } = urInfo
...
function getPrintTemplateTaskInfoUrl (printServiceUrl: string): Promise<TemplateInfoUrlInfo> {
  ...
  return loadArcGISJSAPIModules(['esri/request']).then(modules => {
    ...
    return esriRequest(serverUrl, options).then(res => {
      ...
      let templatesInfoTaskUrl = `${serverUrl}/Get Layout Templates Info Task/${supportedOperations}`
      tasks?.forEach(taskName => {
        if (taskName?.includes('Templates') && taskName?.includes('Info')) {
          templatesInfoTaskUrl = `${serverUrl}/${taskName}/${supportedOperations}`
        }
      })
      return Promise.resolve({
        templatesInfoTaskUrl: `${serverUrl}/Get Layout Templates Info Task`,
        url: templatesInfoTaskUrl,
        executionType: executionType
      })
    })
  })
...
function getAllLayers (layers, LegendLayer): any[] {
  ...
  layers?.forEach(layer => {
    ...
    if (layer.type !== 'graphics' && layer.visible && layer?.legendEnabled) {
