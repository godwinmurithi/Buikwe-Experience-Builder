...
class VersionManager extends BaseVersionManager {
  versions = [{
    ...
    version: '1.11.0',
    description: 'Set "undefined" option to "false"',
    upgrader: async (oldConfig) => {
      let newConfig = oldConfig
      if (newConfig.updateGeometries) {
        newConfig = newConfig.set('updateRecords', true).set('updateGeometries', true).set('updateAttributes', true)
      } else {
        newConfig = newConfig.set('updateRecords', false).set('updateGeometries', false).set('updateAttributes', false)
      }
      return newConfig
    }
  }]
