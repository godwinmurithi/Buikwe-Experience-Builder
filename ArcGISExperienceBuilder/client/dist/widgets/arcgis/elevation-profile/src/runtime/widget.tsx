/** @jsx jsx */
import { React, AllWidgetProps, BaseWidget, jsx, classNames, getAppStore, WidgetState, AppMode, IMState, DataRecord, DataSourceManager, DataSourceStatus } from 'jimu-core'
import {
...
import { JimuMapViewComponent, JimuMapView, FeatureLayerDataSource, geometryUtils } from 'jimu-arcgis'
...
interface IState {
  ...
  updatingChartForSelect: boolean
...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
  ...
  private widgetOpenedWithSelection: boolean
  ...
  constructor (props) {
    ...
    //Once the open widget controller code will work we can uncomment below code
    this.widgetOpenedWithSelection = false
    //if on load we found the selected feature record it means widget is opened with selection message action
    // if (props.mutableStateProps?.selectedFeatureRecords.length > 0) {
    //   this.widgetOpenedWithSelection = true
    // } else {
    //   this.widgetOpenedWithSelection = false
    // }
    ...
    this.state = {
      ...
      profileLineLayers: [],
      lineLayersNotFound: !(activeDsConfig?.advanceOptions && activeDsConfig.profileSettings.layers.length !== 0),
      viewModelErrorState: false,
      profileErrorMsg: '',
      noGraphicAfterFirstSelection: false,
      loadingIndicator: false,
      updatingChartForSelect: false,
    ...
    if (jmv) {
      this.setState({
        ...
        setTimeout(() => {
          ...
          let loadSelectOrDrawTool = true
          //if widgets is opened with the selection from message action lod the selected feature
          //else load select or draw tool on load
          if (this.widgetOpenedWithSelection) {
            this.widgetOpenedWithSelection = false
            //load the selected feature only when the feature is valid and widget is opened
            //If we don't add the widget open condition here in live mode the previously selected feature loads whenever the live mode is toggled
            if (currentWidgetState === WidgetState.Opened && this.props.mutableStateProps?.selectedFeatureRecords?.length > 0) {
              const featureRecord = this.props.mutableStateProps.selectedFeatureRecords?.[0]?.feature
              const isFeatureValid = this.checkSelectedFeaturesAvailability(featureRecord)
              if (isFeatureValid) {
                //when feature is selected dont loadSelectOrDrawTool on load as it may chage few states which dont render the chart at all
                loadSelectOrDrawTool = false
                this.displayFeaturesResult([[featureRecord]])
              }
            }
          }
  ...
  createEpViewModel = (jmv) => {
    const profiles: any = []
    //if view moedel present then empty all the existing profiles and create new profiles
    ...
    //create the view profile only in case of webscenes and when show in graph property in enabled in config
    if (this.state.jimuMapView.view.type === '3d' && this.props.config.configInfo[this.state.currentDatasource]?.profileChartSettings?.showVolumetricObjLineInGraph) {
      profiles.push({
    ...
    reactiveUtils.watch(() => defaultViewModel.chartData, (chartData) => {
      if (!chartData) {
        return
      }
      ...
      this._chartDataTimer = setTimeout(() => {
        ...
        // in case when larger line is selected then the chart data is updated multiple time and it has progress
        // progress is between 0-1, so if we don't skip in between progress the chart flickers as it is rendered multiple times
        if (this._defaultViewModel.state === 'selected' && this._defaultViewModel.progress < 1) {
          if (this._defaultViewModel.progress === 0) {
            this.setState({
              updatingChartForSelect: true,
              viewModelErrorState: false,
              startChartRendering: true,
              profileResult: chartData
            })
          }
          return
        }
        if (this.state.drawModeActive || this.state.selectModeActive) {
          this.setState({
            ...
            if (this.state.updatingChartForSelect) {
              this.setState({
                updatingChartForSelect: false
              })
            }
  ...
  */
  ...
  /**
   * Check features availability selected from the other widgets in the active datasource
  */
  checkSelectedFeaturesAvailability = (selectedFeature): boolean => {
    let isFeatureValid = false
    if (selectedFeature) {
      const dsLayerId = this.getDSLayerID(selectedFeature.layer.id)
      this.getSelectableLayers(this.state.currentDatasource)
      if (selectedFeature?.geometry?.type === 'polyline' && this.selectableLayersAtRuntime.includes(dsLayerId)) {
        isFeatureValid = true
      }
    }
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (currentWidgetState === WidgetState.Opened || !currentWidgetState) {
      ...
      const featureRecord: any = this.props?.selectedFeatureRecords
      if (featureRecord?.length > 0 &&
        ...
        const isFeatureValid = this.checkSelectedFeaturesAvailability(featureRecord?.[0]?.feature)
        if (isFeatureValid) {
          this.displayFeaturesResult([[featureRecord?.[0]?.feature]])
        } else {
          return
        }
      }
    ...
    if (prevConfig?.profileChartSettings.volumetricObjLineColor !== currentConfig?.profileChartSettings.volumetricObjLineColor ||
      ...
      if (this._defaultViewModel && this.state.jimuMapView.view?.type === '3d' && currentConfig?.profileChartSettings.showVolumetricObjLineInGraph) {
        this.createEpViewModel(this.mapView)
      } else if (!currentConfig?.profileChartSettings.showVolumetricObjLineInGraph && this._defaultViewModel?.profiles.length > 1) {
        //remove the view line profile
        this._defaultViewModel.profiles.splice(1, 1)
      }
    }
  }
  ...
  //get output datasource from datasource manager instance
  ...
  buildStatsValuesAsOutput = (profileResult, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip) => {
    ...
    const statsResult = this.outputStatisticsValueDisplay(profileResult, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip)
  ...
  outputStatisticsValueDisplay = (profileResult, outputDs, selectedElevationUnit, selectedLinearUnit, isFlip) => {
    const items = []
  ...
  */
  ...
  resetToDefault = () => {
    //clears the output datasource statistics
        ...
        <div className='widget-elevation-profile'>
          ...
          {this.state.resultStage &&
            <ResultPane
              ...
              displayLoadingIndicator={this.state.loadingIndicator || this.state.updatingChartForSelect}
