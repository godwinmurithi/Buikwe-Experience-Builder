...
export default class Chart extends React.PureComponent<Props, IState> {
  ...
  componentDidUpdate = (prevProps) => {
    ...
    if (prevProps.volumetricObjLineColor !== this.props.volumetricObjLineColor) {
      if (this.chart) {
        const { am4core } = this.amchart4
        this.volumetricObjLineSeries.stroke = am4core.color(this.props.volumetricObjLineColor)
        this.volumetricObjLineSeries.fill = am4core.color(this.props.volumetricObjLineColor)
      }
      return
    }
    ...
    if (profileSettingsChanged ||
      ...
      prevProps.showVolumetricObj !== this.props.showVolumetricObj) {
      if ((prevProps.showVolumetricObj !== this.props.showVolumetricObj ||
        prevProps.profileResult !== this.props.profileResult) && this.chart) {
        this.chart.dispose()
        this.chart = null
      }
        ...
        if (profileOutput.length > 0) {
          ...
          this.createDataForProfileLines(allElevations).then((updatedData) => {
            ...
            if (this.props.isFlip) {
              ...
              let reverseViewOutput
              if (this.viewOutput) {
                reverseViewOutput = [...this.viewOutput].reverse()
              }
              allElevations.forEach((chartData, pointIndex) => {
                ...
                if (reverseViewOutput) {
                  elevationInfo.esriCTViewY = convertSingle(reverseViewOutput[pointIndex].elevation, this.props.profileResult.effectiveUnits.elevation, this.props.selectedElevationUnit)
                }
  ...
  */
  ...
  addVolumetricObjLineSeries = (ctx) => {
    ...
    const viewLineAreaSeries = chart.series.push(new am4charts.LineSeries())
    this.props.chartInfo(this.chart)
    ...
    viewLineAreaSeries.stroke = am4core.color(this.props.volumetricObjLineColor)
    viewLineAreaSeries.fill = am4core.color(this.props.volumetricObjLineColor)
