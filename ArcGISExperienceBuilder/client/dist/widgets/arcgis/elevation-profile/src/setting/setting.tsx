/** @jsx jsx */
import { React, jsx, DataSourceManager, urlUtils, defaultMessages as jimuCoreDefaultMessages, getAppStore, Immutable, FieldSchema, JimuFieldType, DataSourceSchema, DataSourceJson, DataSourceTypes } from 'jimu-core'
  ...
  getInitSchema = (label: string = '') => {
    ...
    epStatistics.forEach((stats) => {
      statsFields.push({
        ...
        jimuName: this.nls(stats.value).replace(/ /g, ''),
    ...
    statsFields?.forEach((fieldSchema, index) => {
      ...
      if (index === 0) {
        const statField = fields as any
        statField.OBJECTID = ObjectIdSchema
      }
    })
