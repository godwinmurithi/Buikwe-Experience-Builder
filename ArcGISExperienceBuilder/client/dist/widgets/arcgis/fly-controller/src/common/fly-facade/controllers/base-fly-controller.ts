...
export default abstract class BaseFlyController {
  ...
  //trackball
        ...
        // let glCamera = new this.GLCamera({ eye: newEyePos, center: _camera.center, up: newUpDir }); //camPos, lookAt, up
        ...
        if (dy != 0 ) {//|| dx != 0
          ...
          let glCamera = new this.GLCamera({ eye: newEyePos, center: _camera.center, up: newUpDirNor }); //camPos, lookAt, up
          let apiCamera = this.cameraUtils.internalToExternal(this.sceneView, glCamera);
          //this.sceneView.camera = apiCamera
