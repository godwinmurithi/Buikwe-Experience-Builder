import {
  AbstractDataAction, DataRecordSet, getAppStore, ImmutableObject, JimuMapViewInfo, defaultMessages,
  MutableStoreManager, utils, i18n, JSAPILayerMixin, DataSource, DataSourceTypes, DataSourceStatus
} from 'jimu-core'
import { MapViewManager, ADD_TO_MAP_DATA_ID_PREFIX, ActionType, DataChangeType, zoomToUtils } from 'jimu-arcgis'
  ...
  async onExecute (dataSet: DataRecordSet, actionConfig?: any): Promise<boolean> {
    ...
    if (jimuMapView) {
      (dataSet.dataSource as unknown as JSAPILayerMixin)?.createJSAPILayerByDataSource().then(layer => {
        zoomToUtils.zoomTo(jimuMapView?.view, layer, null)
      })
    }
