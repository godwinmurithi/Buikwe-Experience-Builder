...
import { t as toAriaBoolean, s as slotChangeHasAssignedElement, b as getElementDir } from './dom-8bd9a2e4.js';
...
const List = class {
  ...
  render() {
    ...
    return (h("div", { class: CSS$1.container }, loading ? h("calcite-scrim", { class: CSS$1.scrim, loading: loading }) : null, h("table", { "aria-busy": toAriaBoolean(loading), "aria-label": label || "", class: CSS$1.table, onKeyDown: this.handleListKeydown, role: "treegrid" }, filterEnabled ? (h("thead", null, h("tr", { class: { [CSS$1.sticky]: true } }, h("th", { colSpan: MAX_COLUMNS }, h("calcite-filter", { "aria-label": filterPlaceholder, disabled: loading || disabled, items: dataForFilter, onCalciteFilterChange: this.handleFilter, placeholder: filterPlaceholder, ref: (el) => (this.filterEl = el), value: filterText }))))) : null, h("tbody", { class: CSS$1.tableContainer }, h("slot", { onSlotchange: this.handleDefaultSlotChange })))));
  }
...
const ListItem = class {
  ...
  renderActionsStart() {
    ...
    return (h("td", { "aria-label": label, class: CSS$2.actionsStart, hidden: !hasActionsStart, key: "actions-start-container", ref: (el) => (this.actionsStartEl = el), role: "gridcell" }, h("slot", { name: SLOTS.actionsStart, onSlotchange: this.handleActionsStartSlotChange })));
  }
  renderActionsEnd() {
    const { label, hasActionsEnd } = this;
    return (h("td", { "aria-label": label, class: CSS$2.actionsEnd, hidden: !hasActionsEnd, key: "actions-end-container", ref: (el) => (this.actionsEndEl = el), role: "gridcell" }, h("slot", { name: SLOTS.actionsEnd, onSlotchange: this.handleActionsEndSlotChange })));
  }
  renderContentStart() {
