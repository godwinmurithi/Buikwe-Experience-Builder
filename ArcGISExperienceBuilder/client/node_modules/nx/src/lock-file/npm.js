...
function stringifyNpmLockFile(lockFileData) {
    ...
    const lockFileJson = Object.assign(Object.assign(Object.assign({}, lockFileData.lockFileMetadata.metadata), (notV1 && {
        packages: (0, sorting_1.sortObject)(packages, (value) => value, false, (a, b) => a.localeCompare(b)),
...
function unmapPackage(packages, dependency) {
    const { packageMeta, rootVersion, version, resolved, integrity, dev, peer, optional } = dependency, value = tslib_1.__rest(dependency, ["packageMeta", "rootVersion", "version", "resolved", "integrity", "dev", "peer", "optional"]);
 ...
 * @returns
 */
function pruneNpmLockFile(lockFileData, packages, projectName) {
    let isV1;
    ...
    const dependencies = pruneDependencies(lockFileData.dependencies, packages, isV1);
    const lockFileMetadata = Object.assign(Object.assign({}, lockFileData.lockFileMetadata), pruneRootPackage(lockFileData, packages, projectName));
    let prunedLockFileData;
    prunedLockFileData = {
        dependencies,
        lockFileMetadata,
        hash: (0, hashing_1.generatePrunnedHash)(lockFileData.hash, packages, projectName),
    };
    return prunedLockFileData;
}
exports.pruneNpmLockFile = pruneNpmLockFile;
function pruneRootPackage(lockFileData, packages, projectName) {
    if (lockFileData.lockFileMetadata.metadata.lockfileVersion === 1) {
        return undefined;
    }
    const rootPackage = Object.assign(Object.assign({ name: projectName || lockFileData.lockFileMetadata.rootPackage.name, version: lockFileData.lockFileMetadata.rootPackage.version }, (lockFileData.lockFileMetadata.rootPackage.license && {
        license: lockFileData.lockFileMetadata.rootPackage.license,
    })), { dependencies: {} });
    for (const packageName of packages) {
        const version = Object.values(lockFileData.dependencies[packageName]).find((v) => v.rootVersion).version;
        rootPackage.dependencies[packageName] = version;
    }
    rootPackage.dependencies = (0, sorting_1.sortObject)(rootPackage.dependencies);
    return { rootPackage };
}
// iterate over packages to collect the affected tree of dependencies
function pruneDependencies(dependencies, packages, isV1) {
    const result = {};
    packages.forEach((packageName) => {
        if (dependencies[packageName]) {
            const [key, _a] = Object.entries(dependencies[packageName]).find(([_, v]) => v.rootVersion), { packageMeta, dev, peer, optional } = _a, value = tslib_1.__rest(_a, ["packageMeta", "dev", "peer", "optional"]);
            result[packageName] = result[packageName] || {};
            result[packageName][key] = Object.assign(value, {
                packageMeta: [{ path: `node_modules/${packageName}` }],
            });
            pruneTransitiveDependencies([packageName], dependencies, result, result[packageName][key], isV1);
        }
        else {
...
function pruneTransitiveDependencies(parentPackages, dependencies, prunedDeps, value, isV1, modifier) {
    let packageJSON;
    if (isV1) {
        ...
        if (versions) {
            ...
            if (dependency) {
                ...
                if (prunedDeps[packageName][key]) {
                    ...
                    pruneTransitiveDependencies([...parentPackages, packageName], dependencies, prunedDeps, prunedDeps[packageName][key], isV1, getModifier(packageMeta));
                }
            }
        }
    });
